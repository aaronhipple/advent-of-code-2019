...
op ← { q ← ⍵[1+((⍺+1)⌷⍵)] ⋄ r ← ⍵[1+((⍺+2)⌷⍵)] ⋄ addr ← 1+⍵[⍺+3] ⋄ result ← q ⍺⍺ r ⋄ (result@addr) ⍵ }
run ← 1∘{ 1=(⍺⌷⍵): (⍺+4)∇(⍺ +op ⍵) ⋄ 2=(⍺⌷⍵): (⍺+4)∇(⍺ ×op ⍵) ⋄ 99=(⍺⌷⍵): ⍵ }
tweak ← { noun verb ← ⍺ ⋄ (noun@2) ((verb@3) ⍵) }
run (12 2 tweak input) ⍝ Solution Part 1

search ← (⊃∘run∘(tweak∘input))¨ ((⍳ 100 100) - 1)
noun verb ← ⊃((⌈/ , ((⍳ 100 100) - 1) ∧ (19690720=search)))
(100×noun)+verb ⍝ Solution Part 2
